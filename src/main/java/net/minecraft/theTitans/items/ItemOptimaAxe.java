...
public class ItemOptimaAxe extends ItemTitanSword {
    public ItemOptimaAxe() {
        super(TitanItems.Tier.Optima, new Properties().setISTER(() -> RenderOptimaAxe::new));
    }
    ...
    public void appendHoverText(ItemStack p_77624_1_, @Nullable World p_77624_2_, List<ITextComponent> p_77624_3_, ITooltipFlag p_77624_4_) {
        ...
        if (p_77624_1_.getDamageValue() > 0) {
            ...
            p_77624_3_.add(ITextComponent.nullToEmpty("\u00A73\u00A7lNormally owned by Clarigatio"));
        } else {
            ...
            p_77624_3_.add(ITextComponent.nullToEmpty("\u00A7l\u00A7k\u00A73\u00A7lNormally owned by Clarigatio"));
        }
    }
            ...
            for (int i1 = 0; i1 < list1.size(); ++i1) {
                ...
                if (entity instanceof IMob || entity instanceof PlayerEntity) {
                    if (player.canSee(entity)) {
                        player.sendMessage(new StringTextComponent("Optima Axe: \u00A7l\u00A7oThe " + entity.getName().getString() + " is " + player.distanceTo(entity) + "\u00A7l\u00A7o blocks away from you."), UUID.randomUUID());
                    } else {
                        player.sendMessage(new StringTextComponent("Optima Axe: \u00A7l\u00A7oAn unseen mob is located out of your \u00A7l\u00A7osight " + player.distanceTo(entity) + " blocks away from you."), UUID.randomUUID());
                        player.sendMessage(new StringTextComponent("Optima Axe: \u00A7l\u00A7oThe mob's health: " + ((LivingEntity) entity).getHealth() + "/" + ((LivingEntity) entity).getMaxHealth()), UUID.randomUUID());
                        player.sendMessage(new StringTextComponent("Optima Axe: \u00A7l\u00A7oThe mob's name: " + entity.getName().getString()), UUID.randomUUID());
            ...
            for (int i2 = -8; i2 <= 8; ++i2) {
                for (int j = -8; j <= 8; ++j) {
                    ...
                    if (block instanceof OreBlock) {
                        player.sendMessage(new StringTextComponent("Optima Axe: \u00A7l\u00A7oI sense a " + block.getName().getString() + "\u00A7l\u00A7o within atleast " + MathHelper.sqrt(player.distanceToSqr(j2, k, m)) + " blocks of you."), UUID.randomUUID());
            ...
            if (list != null && !list.isEmpty()) {
                for (int i1 = 0; i1 < list.size(); ++i1) {
                    ...
                    if (entity != null && entity.isAlive() && entity instanceof LivingEntity && !(entity instanceof EntityTitan) && entity != p_77659_2_) {
                        entity.copyPosition(p_77659_2_);
    ...
    public boolean hurtEnemy(ItemStack p_77644_1_, LivingEntity target, LivingEntity attacker) {
        ...
        if (target != null) {
            ...
            if (target instanceof EntityTitan && (((EntityTitan) target).canBeHurtByPlayer())) {
                ...
                ((EntityTitan) target).setTitanHealth(target.getHealth() - 3000.0f * bonusdamage - extradamage);
    ...
    public void inventoryTick(final ItemStack stack, final World worldIn, final Entity entityIn, final int itemSlot, final boolean isSelected) {
        ...
        if (entityIn instanceof PlayerEntity && !worldIn.isClientSide) {
            if (stack.getDamageValue() == 0 && entityIn.tickCount % 2 == 0) {
                entityIn.sendMessage(new StringTextComponent("Optima Axe: You are unworthy of my power."), UUID.randomUUID());
                ...
                if (((PlayerEntity) entityIn).inventory.contains(new ItemStack(TitanItems.ultimaBlade))) {
    ...
    public boolean onEntitySwing(final ItemStack stack, final LivingEntity entityLiving) {
        ...
        if (!entityLiving.level.isClientSide && entityLiving instanceof PlayerEntity) {
            ...
            final double dx = vec3.x * 16.0;
            final double dy = entityLiving.getEyeHeight() + vec3.y * 16.0;
            final double dz = vec3.z * 16.0;
            for (int k1 = -4; k1 <= 4; ++k1) {
                for (int l1 = -2; l1 <= 2; ++l1) {
                    for (int i2 = -4; i2 <= 4; ++i2) {
                        final int y = MathHelper.floor(entityLiving.position().y + dy + l1);
                        final int x = MathHelper.floor(entityLiving.position().x + dx + k1);
                        final int z = MathHelper.floor(entityLiving.position().z + dz + i2);
        ...
        if (!entityLiving.level.isClientSide && entityLiving instanceof PlayerEntity) {
            for (int i3 = 0; i3 < 12; ++i3) {
                final Vector3d vec3 = entityLiving.getLookAngle();
                final double dx2 = vec3.x * i3;
                final double dy2 = entityLiving.getEyeHeight() + vec3.y * i3;
                final double dz2 = vec3.z * i3;
                final int y = MathHelper.floor(entityLiving.position().y + dy2);
                final int x = MathHelper.floor(entityLiving.position().x + dx2);
                final int z = MathHelper.floor(entityLiving.position().z + dz2);
                if (!entityLiving.level.getBlockState(new BlockPos(x, y, z)).isAir()) {
                    entityLiving.level.levelEvent(null, 1012, new BlockPos(x, y, z), 0);
                }
                if (!entityLiving.level.isClientSide && !entityLiving.level.getBlockState(new BlockPos(x, y, z)).isAir()) {
                    final BlockState block = entityLiving.level.getBlockState(new BlockPos(x, y, z));
                    entityLiving.level.levelEvent(2001, new BlockPos(x, y, z), Block.getId(block));
                    final ItemEntity entityitem = new ItemEntity(entityLiving.level, x, y, z, new ItemStack(block.getBlock().asItem()));
                    entityLiving.level.addFreshEntity(entityitem);
                    entityLiving.level.removeBlock(new BlockPos(x, y, z), true);
                    entityitem.lerpMotion(-MathHelper.sin(entityLiving.yRot * 3.1415927f / 180.0f), 0.75, MathHelper.cos(entityLiving.yRot * 3.1415927f / 180.0f));
                }
        ...
        final Vector3d vec3 = entityLiving.getLookAngle();
        final double dx = vec3.x * 12.0;
        final double dy = entityLiving.getEyeHeight() + vec3.y * 12.0;
        final double dz = vec3.z * 12.0;
        final List list2 = entityLiving.level.getEntities(entityLiving, entityLiving.getBoundingBox().inflate(12.0, 8.0, 12.0).move(dx, dy, dz));
        if (list2 != null && !list2.isEmpty()) {
            for (int i3 = 0; i3 < list2.size(); ++i3) {
                final Entity entity2 = (Entity) list2.get(i3);
                    ...
                    if (entity2 != null) {
                        ...
                        if (entity2 instanceof EntityTitan) {
                            ...
                            if (!((EntityTitan) entity2).canBeHurtByPlayer() && entityLiving instanceof PlayerEntity) {
                                entityLiving.sendMessage(new StringTextComponent(entity2.getName().getString() + ": Quit flailing that giant axe at me."), UUID.randomUUID());
                            }
                            return true;
                        }
                    }
